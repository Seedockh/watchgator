...
class CreateIMDBDatasetService {
    ...
    static moviesExist(level: string): boolean {
      return fs.existsSync(`${this.envSrc}database/imdb/imdb_movies_${level}.json`)
    }

    static seriesExist(level: string): boolean {
      return fs.existsSync(`${this.envSrc}database/imdb/imdb_series_${level}.json`)
    }

    static peoplesExist(level: string): boolean {
      return fs.existsSync(`${this.envSrc}database/imdb/imdb_peoples_${level}.json`)
    }

    static genresExist(level: string): boolean {
      return fs.existsSync(`${this.envSrc}database/imdb/imdb_genres_${level}.json`)
    }
    ...
    static addGenres(media: IMDBMedia | null): void {
      if (media && media.genres) {
        media.genres.map(genre => {
          if (genre) {
            if (genre.name && _.indexOf(this.genresMap, genre.name.trim()) === -1) {
